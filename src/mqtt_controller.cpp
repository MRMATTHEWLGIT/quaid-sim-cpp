...
void MqttController::streamObservations() {
    ...
    while (isStreamingObservations) {
        ...
        float current_front_left = (float)(fabs(d->sensordata[7]) + fabs(d->sensordata[8])) / 100.0f;
        float current_front_right = (float)(fabs(d->sensordata[9]) + fabs(d->sensordata[10])) / 100.0f;
        float current_back_left = (float)(fabs(d->sensordata[11]) + fabs(d->sensordata[12])) / 100.0f;
        float current_back_right = (float)(fabs(d->sensordata[13]) + fabs(d->sensordata[14])) / 100.0f;
        float current = current_front_left + current_front_right + current_back_left + current_back_right;

        StateObservations obs = {
                .time_delta = (int16_t) time_delta,
                .distance = 0,
                .yaw = ypr.yaw, .pitch = ypr.pitch, .roll = ypr.roll,
                .voltage = 5.4, .current = current,
                ...
                .current_front_left = current_front_left,
...
void MqttController::streamMocapData() {
    ...
    while (isStreamingMocap) {
        ...
        MocapData mocapData = {
                ...
                .pitch = ypr.pitch,
                .roll = ypr.roll,
                .qr = (float) d->sensordata[0],
                .qi = (float) d->sensordata[1],
                .qj = (float) d->sensordata[2],
                .qk = (float) d->sensordata[3],
        };
